...
async function search_podcasts({ keywords, max_results = 8 }) {
  ...
  const response = await axios.get(
    'https://listen-api.listennotes.com/api/v2/search',
    {
      ...
      params: { q: keywords, type: 'podcast', per_page: max_results, language: 'English' },
    }
  );
  return {
...
async function generatePitchEmail(podcastName, podcastDescription, podcastAudience, emailNumber, hostName) {
  const client = new Anthropic.Anthropic({ apiKey: process.env.ANTHROPIC_API_KEY });
...
Host name to use: ${hostName || podcastName}

Available pitch angles:
${anglesText}

Instructions:
1. Pick the BEST angle from the list above that fits this podcast's audience
2. Fill in [HOST_NAME] with: ${hostName || 'Hi there'}
3. For Email 1: Fill in [SPECIFIC_TO_PODCAST] with 8-12 words describing what this podcast helps listeners with
4. For Email 2 & 3: Use the chosen angle topic as [EPISODE_TOPIC_ANGLE]
5. Return ONLY the completed email — no extra commentary

Return a JSON object like this:
{
  "chosen_angle": "angle_1",
  "angle_topic": "the full topic text",
  "specific_to_podcast": "the 8-12 word phrase",
  "host_name": "${hostName || podcastName}",
  "email_content": "the complete finished email text"
}
  `.trim();

  // Build the correct email template based on email number
  let templatePrompt = prompt;
  if (emailNumber === 1) {
    templatePrompt += `\n\nGenerate Email 1 using this template:\n${TREVOR_CONTEXT.pitchTemplates.email1('[HOST_NAME]', '[SPECIFIC_TO_PODCAST]')}`;
  } else if (emailNumber === 2) {
    templatePrompt += `\n\nGenerate Email 2 using this template:\n${TREVOR_CONTEXT.pitchTemplates.email2('[HOST_NAME]', '[EPISODE_TOPIC_ANGLE]')}`;
  } else if (emailNumber === 3) {
    templatePrompt += `\n\nGenerate Email 3 using this template:\n${TREVOR_CONTEXT.pitchTemplates.email3('[HOST_NAME]', '[EPISODE_TOPIC_ANGLE]')}`;
  }
...
// ── BUILD PODCAST CARD WITH BUTTONS ─────────────────────
function buildPodcastBlock(podcast, index) {
  ...
  return [
    ...
    {
      type: 'section',
      text: { type: 'mrkdwn', text: `*👥 Audience*\n${podcast.audience || 'Relationship-focused listeners'}` },
    },
    {
      type: 'section',
      text: { type: 'mrkdwn', text: `*💡 Why Trevor fits*\n${podcast.summary || podcast.description}` },
    },
    {
      type: 'actions',
...
function buildEmailBlock(podcastTitle, emailNumber, pitchData, podcastData) {
  ...
  // Add next email button if there is one
  if (hasNextEmail) {
    blocks.push({
      type: 'actions',
      elements: [
    ...
    blocks.push({
      type: 'context',
      elements: [{ type: 'mrkdwn', text: '✅ All 3 emails generated for this podcast.' }],
    });
  }
...
app.post('/slack/find_podcasts', async (req, res) => {
  ...
  try {
    // Get past rejections from database so Claude avoids them
    ...
    // Get past decisions to help Claude learn
...
[
  {
    ...
    const claudeResponse = await askClaudeWithTools(
      `Search for podcasts matching: "${text}". Find the top 5 NEW podcasts for Trevor not already in his pipeline.`,
      systemPrompt
    );
...
app.post('/slack/actions', async (req, res) => {
  ...
  if (actionId === 'reject_podcast') {
    ...
    await db.savePodcastDecision({
      ...
      listenScore: podcastData.listen_score,
      decision: 'rejected',
      decidedBy: userName,
      keywordsSearched: '',
    });
    ...
    await postToSlackChannel(
      ...
      [{
        type: 'section',
        text: { type: 'mrkdwn', text: `❌ *${podcastData.title}* rejected by ${userName} and saved to database.` },
      }],
  ...
  if (actionId === 'approve_podcast') {
    ...
    await postToSlackChannel(
      ...
      [{
        type: 'section',
        text: { type: 'mrkdwn', text: `✅ *${podcastData.title}* approved by ${userName}!\n⏳ Generating Email 1...` },
      }],
    ...
    try {
      ...
      await postToSlackChannel(
        channelId,
        [{ type: 'section', text: { type: 'mrkdwn', text: `❌ Could not generate pitch: ${error.message}` } }],
        'Pitch failed'
      );
    }
  }
  ...
  if (actionId === 'generate_next_email') {
    ...
    const { title, emailNumber, hostName, angleTopic } = data;

    await postToSlackChannel(
      channelId,
      [{
        type: 'section',
        text: { type: 'mrkdwn', text: `⏳ Generating Email ${emailNumber} for *${title}*...` },
      }],
    ...
    try {
      ...
      await postToSlackChannel(
        channelId,
        [{ type: 'section', text: { type: 'mrkdwn', text: `❌ Could not generate Email ${emailNumber}: ${error.message}` } }],
